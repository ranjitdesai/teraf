...
resource "aws_s3_bucket" "desai" {
  bucket        = "my-lambda-bucket-unique-name-1234123"
  acl           = "private"
  force_destroy = true
  tags = {
    Name = "LambdaBucket"
...
resource "aws_lambda_function" "my_lambda" {
  ...
  s3_bucket        = aws_s3_bucket.desai.id # calling code from s3 bucket 
